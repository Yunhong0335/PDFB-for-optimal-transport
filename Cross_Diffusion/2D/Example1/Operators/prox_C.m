function [Q, q1, q2] = prox_C(Q0, q01, q02)
    gam = 1; itermax = 1000;
    
    q11 = q01(:, 1) .* q01(:, 1) + q02(:, 1) .* q02(:, 1);
    q12 = q01(:, 2) .* q01(:, 1) + q02(:, 2) .* q02(:, 1);
    q22 = q01(:, 2) .* q01(:, 2) + q02(:, 2) .* q02(:, 2);
    Lam = cat(2, q11, q12, q12, q22);
        
    Q = Q0; q1 = q01; q2 = q02;
    for ell = 1 : itermax
        RLam = Lam;
        
        q11 = q1(:, 1) .* q1(:, 1) + q2(:, 1) .* q2(:, 1);
        q12 = q1(:, 2) .* q1(:, 1) + q2(:, 2) .* q2(:, 1);
        q22 = q1(:, 2) .* q1(:, 2) + q2(:, 2) .* q2(:, 2);
...
%             ell
        ...
        q11 = (1 + Lam(:, 4)) .* q01(:, 1) - Lam(:, 2) .* q01(:, 2);
        q12 = (1 + Lam(:, 1)) .* q01(:, 2) - Lam(:, 3) .* q01(:, 1);
        
        q21 = (1 + Lam(:, 4)) .* q02(:, 1) - Lam(:, 2) .* q02(:, 2);
        q22 = (1 + Lam(:, 1)) .* q02(:, 2) - Lam(:, 3) .* q02(:, 1);
        
        Q = Q0 - Lam;
        q1 = cat(2, q11./det, q12./det); q2 = cat(2, q21./det, q22./det);
    end
end
